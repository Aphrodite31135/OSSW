...
class AssetPipeline:
    ...
    def _generate_with_hunyuan_api(
        ...
        source_image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        source_image = ImageOps.exif_transpose(source_image)
        preview_image = self._prepare_texture(source_image, 512)
        ...
        return {
            ...
            "metadata_url": f"/outputs/{job_id}/{metadata_path.name}",
            "summary": (
                "Generated a GLB asset through the real image-to-3D backend. "
                "The package includes the model file, preview image, grayscale render, and metadata."
            ),
        }
